...
$objFieldInfo->getList(" AND sub_cat_id='".SUB_TC_TCDT_THONGTIN_NGUOI_DAIDIEN_DOITAC."' AND isactive='1'", " ORDER BY sort asc");
$icon = "";
$i=0;
$totalRecord = $objFieldInfo->Num_rows();

// Lấy alias cub category 
$aliasSubCat = trim($objSubCat->getAliasSubCat(SUB_TC_TCDT_THONGTIN_NGUOI_DAIDIEN_DOITAC));
...
if (count($arrayData) > 0) {	
	foreach ($arrayData as $k => $strJson) {
		...
		if (isset($dataEdit[$aliasSubCat]) && $dataEdit['sub_cat_id'] == SUB_TC_TCDT_THONGTIN_NGUOI_DAIDIEN_DOITAC) {
			break;
		}		
	}
}
...
?>
	...
	<section class="col col-3 <?php if($rs['alias']=='menh') echo "field_hidden";?>">
		...
		<label class="input">
			...
			<input type="text" name="gr12_<?php echo SUB_TC_TCDT_THONGTIN_NGUOI_DAIDIEN_DOITAC;?>_<?php echo $rs['alias']?>[]"value="<?php echo $valueData;?>">
		</label>	
	</section>
	<?php endif;?>
		...
		<div class="inline-group">
			...
			<?php if ($dataEdit != ""):?>
				<?php
					...
					while ($r=$objOptionField->Fetch_Assoc()) { ?>
						<label class="radio">
							<input type="radio" name="gr12_<?php echo SUB_TC_TCDT_THONGTIN_NGUOI_DAIDIEN_DOITAC;?>_<?php echo $rs['alias']?>[]" value="<?php echo $r['value']?>" 
			...
			<?php else:?>
				<?php
					...
					while ($r=$objOptionField->Fetch_Assoc()) { ?>
						<label class="radio">
							<input type="radio" name="gr12_<?php echo SUB_TC_TCDT_THONGTIN_NGUOI_DAIDIEN_DOITAC;?>_<?php echo $rs['alias']?>[]" value="<?php echo $r['value']?>" <?php if($m==0) echo "checked=\"true\""?>>
							<i></i><?php echo $r['name'];?>
						</label>
					<?php $m++;}
				?>
	...
	<section class="col col-3">
		...
		<label class="textarea">
			...
			<textarea name="gr12_<?php echo SUB_TC_TCDT_THONGTIN_NGUOI_DAIDIEN_DOITAC;?>_<?php echo $rs['alias']?>[]"><?php echo $valueData;?></textarea>											
		</label>
	</section>
	<?php endif; ?>
	...
	<section class="col col-3">
		...
		<label for="file" class="input input-file">
			<div class="button">
				<input type="file" multiple="multiple" name="gr12_<?php echo SUB_TC_TCDT_THONGTIN_NGUOI_DAIDIEN_DOITAC;?>_<?php echo $rs['alias']?>_files[]"  class="link"
		...
		<?php
		if ($dataEdit != "") { ?>
			<input type="hidden" name="gr12_<?php echo SUB_TC_TCDT_THONGTIN_NGUOI_DAIDIEN_DOITAC.'_'.$rs['alias']?>" value="<?php echo $valueData;?>">
		<?php }
		?>
	</section>
	<?php endif; ?>
	...
	<section class="col col-3">
		<label class="label"><?php echo $rs['name']?></label>
		<label class="select">
			<select name="gr12_<?php echo SUB_TC_TCDT_THONGTIN_NGUOI_DAIDIEN_DOITAC;?>_<?php echo $rs['alias']?>[]">
				<?php
					$fieldId = $rs['id'];
					$objOptionField->getList(" AND field_id=$fieldId", "");
					while ($r=$objOptionField->Fetch_Assoc()) { ?>
